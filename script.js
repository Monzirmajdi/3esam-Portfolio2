...
function addAnimation() {
    ...
    const projectImageCloseBtn = document.getElementById("projectImageCloseBtn");
    const projectImageBackBtn = document.getElementById("projectImageBackBtn");

    // Function to open project image modal
    function openProjectImageModal(imageSrc, projectTitle = "") {
        if (projectImageModal && projectImageDisplay) {
            projectImageDisplay.src = imageSrc;
            projectImageDisplay.alt = projectTitle || "Project Image";
            projectImageModal.classList.add("active");
            document.body.style.overflow = "hidden";
        }
    }

    // Function to close project image modal
    function closeProjectImageModal() {
        if (projectImageModal) {
            projectImageModal.classList.remove("active");
            document.body.style.overflow = "auto";
    ...
    if (projectImageCloseBtn) {
        projectImageCloseBtn.addEventListener("click", closeProjectImageModal);
    }

    // Back button event listener
    if (projectImageBackBtn) {
        projectImageBackBtn.addEventListener("click", function() {
            closeProjectImageModal();
            // Optionally open the portfolio modal or scroll to projects section
    ...
    // Add click event listeners to project category images
    const projectCategoryImages = document.querySelectorAll(".project-category-image");
    projectCategoryImages.forEach(image => {
        image.addEventListener("click", function(e) {
            e.preventDefault();
            e.stopPropagation();
            const imageSrc = this.src;
            const projectTitle = this.alt || "Project Image";
            openProjectImageModal(imageSrc, projectTitle);
        });
    });

    // Add keyboard support for the project image modal
    document.addEventListener("keydown", function(e) {
        if (projectImageModal && projectImageModal.classList.contains("active")) {
            if (e.key === "Escape") {
                closeProjectImageModal();
            }
        }
    ...
    // Make the function globally available for other uses
